...
bool is_directory_exist(char * path) {
    ...
    if(lstat(path, &info) != 0) {
        if(errno == ENOENT) {
            ...
            daemon_log(LOG_ERR, "%s %s %s", __FUNCTION__, path, strerror(errno));
            return false;
        }
        return false;
    }
    ...
    daemon_log(LOG_ERR, "%s it exists but it's a file or symlink or namedpipe", path);
    return false;
}
...
const char * daemon_pid_file_proc_default(void) {
    ...
    snprintf(fn, sizeof(fn), "%s/%s/%s.pid", VARRUN, tmp_p, tmp_p);
    return fn;
}
